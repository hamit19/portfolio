...
const Footer = ({ setOpen }) => {
    ...
    return subFooterSocialMedias.map((social) => (
      <a href={social.link} key={social.id} target="_blank" rel="noreferrer">
        <div className="flex flex-row gap-4 py-4 cursor-pointer group ">
          <div className="p-3 border rounded-full group-hover:shadow-md group-hover:text-indigo-300 custom-transition group-hover:shadow-indigo-300/50 text-gray-50">
            {social.icon}
          </div>
          <div className="flex flex-col pt-1 ">
            <h5 className="text-xs font-lg group-hover:text-shadow group-hover:text-indigo-300 custom-transition text-gray-50">
              {" "}
              {social.title}
            </h5>
            <p className="text-sm font-lg group-hover:text-shadow group-hover:text-indigo-300 custom-transition text-gray-50">
              {social.content}
            </p>
          </div>
        </div>
  ...
  return (
    <div id="footer" className="bg-slate-900">
      ...
      <div>
        <h3 className="py-8 text-3xl text-center font-lg text-gray-50">
          Follow Me
        </h3>
      ...
      <div className="py-6 border-t border-b border-t-slate-500 border-b-slate-500 ">
        <h3 className="text-sm text-center font-lg text-gray-50">
